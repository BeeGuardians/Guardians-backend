...
public class BadgeController {
    ...
    public ResponseEntity<ResWrapper<?>> getUserBadges(@PathVariable Long userId) {
        return ResponseEntity.ok(
                ResWrapper.resSuccess("유저 뱃지 목록 조회 성공", badgeFacade.getAllBadgesWithUserStatus(userId))
        );
    }
    ...
    public ResponseEntity<ResWrapper<?>> getEarnedBadges(@PathVariable Long userId) {
        return ResponseEntity.ok(
                ResWrapper.resSuccess("획득 뱃지 조회 성공", badgeFacade.getEarnedBadges(userId))
        );
    }
