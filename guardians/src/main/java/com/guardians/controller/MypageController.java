...
public class MypageController {
    ...
    public ResponseEntity<ResWrapper<?>> getProfile(@PathVariable Long userId) {
        return ResponseEntity.ok(ResWrapper.resSuccess("마이페이지 조회 성공", rankingFacade.getProfile(userId)));
    }
    ...
    public ResponseEntity<ResWrapper<?>> getSolvedProblems(@PathVariable Long userId) {
        var result = rankingFacade.getSolvedProblems(userId);
        int count = result.getSolvedList().size();
        return ResponseEntity.ok(ResWrapper.resList("푼 문제 조회 성공", result, count));
    }
    ...
    public ResponseEntity<ResWrapper<?>> getBookmarks(@PathVariable Long userId) {
        var result = rankingFacade.getBookmarks(userId);
        int count = result.getBookmarks().size();
        return ResponseEntity.ok(ResWrapper.resList("북마크 조회 성공", result, count));
    }
    ...
    public ResponseEntity<ResWrapper<?>> getBoards(@PathVariable Long userId) {
        var result = rankingFacade.getBoards(userId);
        int count = result.getBoards().size();
        return ResponseEntity.ok(ResWrapper.resList("내 게시글 조회 성공", result, count));
    }
    ...
    public ResponseEntity<ResWrapper<?>> getReviews(@PathVariable Long userId) {
        var result = rankingFacade.getReviews(userId);
        int count = result.getReviews().size();
        return ResponseEntity.ok(ResWrapper.resList("내 리뷰 조회 성공", result, count));
    }

    @Operation(summary = "내 랭킹 조회", description = "현재 내 랭킹 정보 조회")
    @GetMapping("/rank")
    public ResponseEntity<ResWrapper<?>> getRank(@PathVariable Long userId) {
        return ResponseEntity.ok(ResWrapper.resSuccess("내 랭킹 조회 성공", rankingFacade.getRank(userId)));
    }
    ...
    public ResponseEntity<ResWrapper<?>> getUserStats(@PathVariable Long userId) {
        return ResponseEntity.ok(
                ResWrapper.resSuccess("내 통계 정보 조회 성공", rankingFacade.getUserStats(userId))
        );
    }
