...
interface JpaSolvedWargameRepository extends JpaRepository<SolvedWargame, SolvedWargameId> {
    ...
    @Query("SELECT s FROM SolvedWargame s JOIN FETCH s.wargame w JOIN FETCH w.category WHERE s.user.id = :userId")
    ...
    """)
    List<SolvedWargame> findByUserIdAndCategoryName(@Param("userId") Long userId, @Param("categoryName") String categoryName);
    ...
    @Query("SELECT w.category.name, COUNT(sw) FROM SolvedWargame sw JOIN sw.wargame w WHERE sw.user.id = :userId GROUP BY w.category.name")
